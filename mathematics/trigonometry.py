...
def in_domain_sin(x):
    ...
    '''
    return not ((x > general.f1 + general.epsilon) or ( x < - general.f1 - general.epsilon))
...
def check_in_domain_sin(x):
    ...
    y = x
    if x > general.f1:
        assert x < general.f1 + general.epsilon
        y = general.f1
    elif x < - general.f1:
        assert x > - general.f1 - general.epsilon
        y = - general.f1
...
def angle_standard_range(angle_rad):
    ...
    '''
    if abs(angle_rad - math.pi) < general.epsilon:
        return math.pi
            
    if abs(angle_rad + math.pi) < general.epsilon:
        return - math.pi 

    if (angle_rad > -general.pi) and (angle_rad < general.pi):
        return angle_rad
    else:
        mod_two_pi = reminder_two_pi(angle_rad)
        if mod_two_pi > general.pi :
            mod_mpi_ppi = mod_two_pi - general.two_pi
        else :
...
def arcsincos(s, c):
    ...
    '''
    assert general.equal(s**2 + c**2, 1.0) 
    if s < 0:                                                        
        theta = - arccos(c)
    else:
        theta = arccos(c)
...
def arctan_frac(x,y):
    ...
    If y is very close to 0.0 (closer than general.epsilon), one of the values - pi/2 or pi/2 is returned depending on the sign of x
    '''
    if (y < general.epsilon) and (y >= 0.0):
        if x > 0:
            return 0.5*math.pi
        else:
            return  - 0.5*math.pi
    elif (y > - general.epsilon) and (y <= 0.0):
        if x > 0:
            return  - 0.5*math.pi
        else:
            return 0.5*math.pi
...
def solve_equation(code, parameters):
    ...
    if code == 1:
        ...
        assert (not general.equal(R, 0))
        s = - parameters[2] / R
        if in_domain_sin(s):
            sai0 = math.atan2(parameters[0], parameters[1])
            if general.equal(math.sin(sai0), parameters[0]/R):
                sai = sai0
            ...
            if general.equal(parameters[0]*math.cos(t1) + parameters[1]*math.sin(t1) + parameters[2] , 0):
                solution_set.append(t1)
            
            if general.equal(parameters[0]*math.cos(t2) + parameters[1]*math.sin(t2) + parameters[2] , 0):
...
def solve_system(code,A,B,x,y,z):
    ...
    for i in range(0,len(t0)): 
        for j in range(0,len(t1)):   
            ...
            c0 = math.cos(t0[i])
            s0 = math.sin(t0[i])
            if (general.equal(x , c0*q) and
               general.equal(y , s0*q) and
               general.equal(z , p)):
