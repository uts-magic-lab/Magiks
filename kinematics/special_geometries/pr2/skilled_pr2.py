...
Last Revision:  17 September 2014
...
class Skilled_PR2(ps.PyRide_PR2):
    ...
    def writing_posture(self, d = 0.6):
        pos = np.array([ d-0.05,  0.3,  0.0])
        ori = ps.rot.point_forward_orientation 
        self.larm.set_target(pos, ori)
        self.reach_target_larm(wait = False)

        pos = np.array([ d-0.05,  - 0.3,  0.0])
        self.rarm.set_target(pos, ori)
    ...
    def draw_from_canvas(self, canvas_shapes, height = 0.2, width = 0.3, width_pixels = 640, height_pixels = 480, plot = False, silent=True):
        self.sync_object()
        direction = self.endeffector_orientation()
        w = - direction[:,0]
        h = direction[:,1]
        n = direction[:,2]
        ...
        for cs in canvas_shapes:
            if cs.arm == "R":
                phi     = 1.0
                (x0,y0) = cs.pts[0]
                pos0    = rarm_lower_left + width*x0*w/width_pixels + height*(height_pixels-y0)*h/height_pixels
                pr      = traj.Polynomial_Trajectory(dimension = 3)
                pr.add_point(0.0, np.copy(pos0), np.zeros(3))
                for (x,y) in cs.pts[1:]:
                    pos  = rarm_lower_left + width*x*w/width_pixels + height*(height_pixels-y)*h/height_pixels
                    pr.add_point(phi, pos)
                    pr.new_segment()
                    phi += 1.0
                # pr.adjust_phase_by_distance(gain = 100.0)
                ...
                phi     = 1.0
                (x0,y0) = cs.pts[0]
                pos0    = larm_lower_left + width*x0*w/width_pixels + height*(height_pixels-y0)*h/height_pixels
                pl      = traj.Polynomial_Trajectory(dimension = 3)
                pl.add_point(0.0, np.copy(pos0), np.zeros(3))
                for (x,y) in cs.pts[1:]:
                    pos  = larm_lower_left + width*x*w/width_pixels + height*(height_pixels-y)*h/height_pixels
                    pl.add_point(phi, pos)
                    pl.new_segment()
                    phi += 1.0
                # pl.adjust_phase_by_distance(gain = 100.0)
    ...
    def write_A(self):
        ...
        if self.write_from_shape_complete:
            A_complete = tsh.A(width = self.width, height = self.height, direction = rot.point_forward_orientation, complete = True)
            self.arm_trajectory(A_complete, speed = self.writing_speed, delta_phi = 0.5)
        else:
            self.arm_forward(self.depth)
            if self.write_from_shape:
                A = tsh.A(width = self.width, height = self.height, direction = rot.point_forward_orientation)
                self.arm_trajectory(A, speed = self.writing_speed, delta_phi = 0.5)
            else:
                self.arm_right_up(dx   = 0.4*self.width, dy = self.height, speed = self.writing_speed)
                self.arm_right_down(dx = 0.4*self.width, dy = self.height, speed = self.writing_speed)

            self.arm_back(self.depth, speed = self.writing_speed/2)
            self.arm_left_up(0.6*self.width, 0.5*self.height, speed = self.writing_speed)
            self.arm_forward(self.depth, speed = self.writing_speed/2)
            self.arm_right(0.4*self.width, speed = self.writing_speed)
            self.arm_back(self.depth, speed = self.writing_speed/2)
            self.arm_right_down(0.3*self.width, 0.5*self.height, speed = self.writing_speed)

    def write_B(self):
        if self.write_from_shape_complete:
            B_complete = tsh.B(width = 0.4*self.height, height = self.height, direction = rot.point_forward_orientation, complete = True)
            self.arm_trajectory(B_complete, speed = self.writing_speed, delta_phi = 0.5)
        else:
            self.arm_forward(self.depth, speed = self.writing_speed/2)
            if self.write_from_shape:
                B = tsh.B(width = 0.4*self.height, height = self.height, direction = rot.point_forward_orientation)
                self.arm_trajectory(B, speed = self.writing_speed, delta_phi = 0.5)
            else:
                self.arm_up(self.height, speed = self.writing_speed)
                self.arm_arc(center = np.array([0.0, 0.0, - 0.25*self.height]), speed = self.writing_speed)
                self.arm_arc(center = np.array([0.0, 0.0, - 0.25*self.height]), speed = self.writing_speed)
            self.arm_back(self.depth, speed = self.writing_speed/2)
            self.arm_right(0.4*self.height, speed = self.writing_speed)
